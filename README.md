...
- 자동 의존관계 주입을 기본으로 사용하면 좋음
- 수동 빈 등록은 언제 사용하면 좋을까?

- 업무 로직 빈: 숫자가 굉장히 많음. 자동 등록을 적극 사용 권함. 
- 기술 지원 빈: 업무 로직 빈에 비해 숫자가 적음. 예) 공통 로그 처리. 수동 등록 적극 사용함으로써 명확하게 드러내는 것이 좋음.

- 애플리케이션에 광범위하게 영향을 미치는 빈은 수동 등록함으로써 설정 정보에 딱 나타나게 하는 것이 유지보수에 좋음.
- 이런 빈들을 컴포넌트 스캔을 한다면, 매뉴얼을 정해놓거나 위치를 지정을 해줘야 함.

- 기본적으로 자동 빈 등록을 사용하자
- 직접 등록하는 기술 지원 객체를 수동으로 등록하자
- 다형성을 적극 활용하는 비즈니스 로직은 수동 등록을 고민해보자

<br>

## 빈 스코프
- 싱글톤 스코프: 기본 스코프. 스프링 컨테이너와 생명주기가 같음.
- 프로토타입 스코프: 스프링 컨테이너가 빈 생성, 의존관계 주입까지만 관여하고 추가적인 관리를 하지 않는 스코프.
- 웹 관련 스코프
